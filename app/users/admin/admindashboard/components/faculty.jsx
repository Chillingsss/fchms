...
const FacultyManagement = () => {
	...
	const fetchRoleadmin = async () => {
		try {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/role/fetch-role.php`
			);
	...
	const handleUsersSubmit = async (e) => {
		...
		try {
			const response = await axios.post(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/useraccounts/add-account.php`,
				formData,
				{
					headers: {
	...
	const fetchuseraccounts_info = async () => {
		try {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/useraccounts/fetch-account.php`
			);
	...
	const handleView = async (user_id) => {
		try {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/useraccounts/view-account.php?user_id=${user_id}`
			);
			if (response.data.success) {
	...
	const handleUsersUpdateSubmit = async () => {
		...
		try {
			...
			const { data } = await axios.post(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/useraccounts/edit-account.php`,
				formData,
				{
					...
					<div className="flex items-center justify-between pt-6 mb-4">
						<div className="flex gap-2 items-center">
							...
							<div className="relative inline-block w-fit">
								...
								{/* Icon on the right */}
								<FiFilter className="absolute right-3 top-1/2 -translate-y-1/2 text-black pointer-events-none h-5 w-5" />
							</div>
						</div>
								...
								<div className="w-20 h-20 rounded-full overflow-hidden border-2 border-gray-300">
									<img
										src={
											editImagePreview
												? editImagePreview
												: `http://localhost/${Users.photo_url}`
										}
										alt="User Profile"
										className="w-full h-full object-cover"
									/>
									...
									<div className="w-20 h-20 rounded-full overflow-hidden border-2 border-gray-300">
										<img
											src={
												editImagePreview
													? editImagePreview
													: `http://localhost/${UserData.photo_url}`
											}
											alt="User Profile"
											className="w-full h-full object-cover"
										/>
