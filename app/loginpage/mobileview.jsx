...
export default function AdminLogin() {
	...
	const generateCaptcha = () => {
		...
		for (let i = 0; i < captcha.length; i++)
			ctx.fillText(captcha[i], startX + i * spacing, startY);
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await axios.post(
				`${process.env.NEXT_PUBLIC_API_BASE_URL}/loginphp/loginform.php`,
				{ email, password },
				{
					headers: {
	...
	return (
		<div className="flex flex-col lg:flex-row min-h-screen">
				...
				<div className="relative z-10 w-full h-full min-h-[16rem] lg:min-h-screen bg-green-800/85 text-white p-6 flex flex-col">
					<div className="flex flex-col lg:flex-row items-center justify-center flex-grow gap-6 text-center lg:text-right">
						...
						>
							...
							<h2 className="text-sm lg:text-md mt-1 font-semibold">
						...
						{/* Divider */}
						<motion.div
							className="hidden lg:block w-px h-28 bg-white"
							custom={4}
							variants={leftItemVariants}
						/>
						...
						{/* Logo */}
						<motion.div
							className="w-40 h-20 lg:w-[400px] lg:h-[160px] mt-4 lg:mt-0 mb-52 lg:mb-52"
							custom={5}
							variants={leftItemVariants}
						>
							<Image
			...
			<div className="w-full lg:w-1/2 flex flex-col items-center justify-center bg-white relative overflow-hidden px-4 py-6">
				...
				{/* Login Form */}
				<motion.div
					className="w-full max-w-md sm:max-w-lg lg:max-w-xl p-6 sm:p-10 bg-white rounded-lg shadow-2xl z-10"
					...
					>
						<motion.div variants={itemVariants}>
							...
							<div className="relative">
								...
								<Input
									...
									className="pl-10 w-full h-11 text-sm border border-green-800 bg-transparent text-black rounded-md shadow-xl"
						...
						<motion.div variants={itemVariants}>
							...
							<input
								...
								className="w-full p-2 h-11 border border-black shadow-xl rounded-md bg-white text-black text-sm"
						...
						<motion.div variants={itemVariants}>
							<Button
								type="submit"
								disabled={isLocked}
								className={`w-full h-11 rounded-md flex items-center justify-center gap-2 shadow-xl ${
				...
				<div className="pt-4 pb-8">
					<motion.footer
						variants={itemVariants}
						className="text-green-800 font-semibold text-base sm:text-lg lg:text-2xl text-center"
					>
